...
ui <- fluidPage(
  ...
  navlistPanel(
    "Choose Output!",
    tabPanel("Table", 
             selectInput("Food_Type1", "Food Type", c("All","Vegetarian"), selected = "All"),
             DTOutput("Food_Impacts_Table")),
    tabPanel("Plots",
             selectInput("Food_Type2", "Food Type", c("All","Vegetarian"), selected = "All"),
...
server <- function(input, output, session) { 
  ...
  df2 <- reactive({
    ...
    output$Food_Impacts_Table <-renderDT(df1(), options = list(lengthChange = TRUE))
